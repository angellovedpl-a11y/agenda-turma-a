...
**Hotfix SSL "bad record mac" — pool DB compartilhado entre workers via fork** (`gunicorn.conf.py` novo + `.replit`; 01/05/2026)
- **Sintoma:** logo após o hotfix anterior subir, app travou completamente — login impossivel. Logs prod mostravam erros NOVOS e diferentes em massa: `SSL error: ssl/tls alert bad record mac`, `SSL error: decryption failed or bad record mac`, `SSL SYSCALL error: EOF detected`, `[kvstore] retry load(sessions) tentativa 2/3 (conn morta)` esgotando os 3 retries — toda chamada DB falhava.
- **Causa raiz (bug clássico psycopg2 + gunicorn `--preload`):** o flag `--preload` (adicionado na Fase 3 Bloco A pra acelerar boot) faz o master importar `server.py` UMA vez antes de fazer fork dos workers. Esse import dispara `kvstore._kv_init.init_schema()` (server.py:55), que chama `_connect()` e cria o pool psycopg2 com 2 conexões SSL abertas no MASTER process. Quando o master roda `os.fork()` x2, os file descriptors SSL são DUPLICADOS entre os 2 workers — ambos passam a usar as mesmas conexões SSL com o mesmo estado criptográfico (chaves de sessão, contadores de bloco). O openssl detecta que dois lados estão escrevendo no mesmo stream com contadores fora de sincronia e aborta com "bad record mac" / "decryption failed". Não é um bug do nosso código — é uma incompatibilidade fundamental entre `os.fork()` e qualquer socket TLS aberto. As proteções do hotfix anterior (keepalives, retry) NÃO ajudam porque cada nova conexão criada por um worker pra "substituir" a morta cai no mesmo problema (o pool inteiro tá envenenado).
- **Fix:** novo arquivo `gunicorn.conf.py` define o hook `post_fork(server, worker)` que roda DENTRO de cada worker depois do fork, chamando `kvstore.close_pool()`. Isso força o `_get_pool()` a recriar um pool novo e exclusivo daquele worker (com sockets SSL frescos abertos pelo próprio worker, sem compartilhamento com nenhum outro processo). Padrão reconhecido pra psycopg2/redis-py/SQLAlchemy + gunicorn `--preload`. `.replit` `[deployment].run` atualizado pra incluir `-c gunicorn.conf.py`.
- **`--preload` MANTIDO:** removê-lo causaria a regressão do boot timeout (Bloco F da Fase 3) — em Reserved VM 0.5–1 vCPU os 2 workers tentam importar `server.py` em paralelo competindo por CPU, demoram 30-60s, o load balancer marca unhealthy. Solução correta é manter `--preload` E resetar o pool pos-fork.
- **Aditividade:** arquivo novo, isolado. Não altera `kvstore.py`, `auth.py` ou nenhum código de aplicação. Reverter é trivial: remover `gunicorn.conf.py` e tirar `-c gunicorn.conf.py` do `.replit`. Comportamento sem o hook = comportamento pre-hotfix (quebrado), com o hook = comportamento esperado.

**Hotfix login fantasma — "matricula incorreta" nas 1as tentativas** (`kvstore.py` + `auth.py`; 01/05/2026)
- **Sintoma:** dono e outros usuários reclamavam que ao logar com matrícula+senha CORRETAS, as primeiras 1–2 tentativas voltavam "Matricula ou senha incorretas", e só na 3ª tentativa entrava — sem trocar nada nos campos.
- **Causa raiz (logs de prod confirmaram):** `[kvstore] erro load(users): SSL connection has been closed unexpectedly` + `[kvstore] erro load(sessions): SSL SYSCALL error: EOF detected`. O Postgres do Replit (ou middlebox de rede) fechava conexões SSL idle do pool sem o cliente saber. O pool tinha conexões "zombie" e a 1ª/2ª chamada após período ocioso falhava silenciosamente em `kvstore.load('users')`. Como `load()` engolia a exceção e devolvia `{}`, o `auth.handle_login` via `users.get(matricula) → None` e retornava 401 "Matricula ou senha incorretas" — mensagem ENGANOSA, parecia erro de credencial mas era erro de DB. A 3ª tentativa funcionava porque o `_connect()` já havia descartado as conexões mortas (com `close=True`) e o pool servia uma fresca.
- **Fix em 3 frentes (todas aditivas):**
  - **Frente 1 — TCP keepalives no pool DB** (`_get_pool` em `kvstore.py`): pool agora abre com `keepalives=1, keepalives_idle=30, keepalives_interval=10, keepalives_count=5`. O kernel manda probes TCP a cada 30s de idle, evitando que o servidor/middlebox derrube a conexão SSL silenciosamente. **Preventivo.**
  - **Frente 2 — Retry automático em `load()` e `save()`** (`kvstore.py`): se a conexão pega do pool estiver morta (`psycopg2.InterfaceError`/`OperationalError`), retenta até 3x. O `_connect()` já descarta conn ruim com `close=True`, então a próxima iteração pega uma fresca. `save()` é UPSERT (`ON CONFLICT DO UPDATE`), então retry é idempotente. Quando `conn` vem de fora (`with_lock`), retry é desabilitado pra não romper transação. **Corretivo.**
  - **Frente 3 — `handle_login` distingue 503 de 401** (`auth.py`): chama `kvstore.load('users', raise_on_error=True)` e em caso de `KVStoreError` retorna **503 "Servidor temporariamente indisponivel, tente novamente em instantes"** em vez do 401 enganoso. Mesmo se as 3 frentes acima falharem juntas, o user vê mensagem CORRETA ("DB fora", não "senha errada") e sabe o que fazer. **Defensivo.**
- **Aditividade:** keepalives são opções padrão do libpq desde sempre; não mudam comportamento, só mantêm conexão viva. Retry só dispara em conexão morta (cenário que antes resultava em erro silencioso); sucesso na 1ª tentativa é idêntico ao antes. Mensagem 503 só aparece se TODOS os 3 retries falharem — caso onde antes o user via 401 enganoso. Reverter qualquer das 3 frentes é trivial (git diff curto e isolado).

**Hotfix tema claro — backgrounds escuros vazando** (`index.html`; 30/04/2026)
- **Sintoma:** dono reportou que no modo claro vários modais/toasts/caixas ficavam com fundo preto e texto ilegível. Caso emblemático: ao tentar upload no acervo, a notificação de erro só mostrava o "X vermelho", o resto do texto sumia em fundo escuro.
- **Causa raiz:** o app tem dois temas (`[data-theme="dark"]` default e `[data-theme="light"]` opcional) usando variáveis CSS, mas dezenas de regras e estilos inline tinham cores HARDCODED (`#0e1117`, `#0f1117`, `#0a0d14`, `#0a0c10`, `#161a22`, `#15171d` etc) sem override pro tema claro. As mais críticas: `.toast` (1593), `.vp-opt:hover` (1592), `.tabs-mini button` (1609), `.btn-danger` (1602), `.btn-legend`/`.theme-toggle` (324/338), `.wk` (395), `#vwPlusPop` (popup do "+" no chat Viriato), formulários admin de "regra técnica"/"apelo de erro" (caixas + textareas) e o viewer fullscreen do manual PDF.
- **Fix em 2 frentes (totalmente aditivo):**
  - **Frente A — overrides CSS pra modo claro:** bloco novo no `injectCSS()` (~linha 1614) cobrindo `.toast`, `.vp-opt`, `.tabs-mini button`, `.btn-danger`, `.btn-legend`, `.theme-toggle`, `.wk`, `#vwPlusPop`. Dark continua idêntico — só adicionou regras `[data-theme="light"] X{...}`.
  - **Frente B — refatorar inline styles pra usar variáveis do tema:** trocas pontuais em estilos inline (caixas dos modais admin de regra/apelo, viewer manual) de `#0f1117` → `var(--card-2)`, `#0a0d14` → `var(--bg)`, `#0e1117` → `var(--card)`, `#161a22` → `var(--card-2)`. As variáveis no dark resolvem em cores quase idênticas às originais (delta visual mínimo); no claro resolvem em branco/cinza-claro corretos.
- **Aditividade:** zero remoção, dark mode preservado bit-a-bit nas regras com override; nas refatoradas a diferença é imperceptível (`#0f1117` → `#141414`, etc.). Reverter é trivial (git diff curto).
