  ...
  `,async(body)=>{
    ...
    const load=async()=>{
      ...
      if(tab==="pend"){
        ...
        p.innerHTML=list.map(u=>`<div class="lst-item"><div style="flex:1"><div style="font-weight:600">${escapeHtml(u.nome||"")}</div><div style="font-size:12px;color:var(--muted)">Mat ${escapeHtml(u.matricula||"")} · ${escapeHtml(u.role||"membro")}${u.status==="negado"?' · <span style="color:#ff6b6b;font-weight:700">BANIDO</span>':u.status==="pendente"?' · <span style="color:var(--orange,#f5a623)">pendente</span>':''}</div></div><div style="display:flex;gap:4px;flex-wrap:wrap">${u.role==="admin"||u.role==="aprovador"?'<button class="btn-secondary" data-act="dp" data-m="'+escapeHtml(u.matricula)+'" style="padding:4px 8px;font-size:11px">↓ Despromover</button>':'<button class="btn-secondary" data-act="pr" data-m="'+escapeHtml(u.matricula)+'" style="padding:4px 8px;font-size:11px">↑ Promover</button>'}<button class="btn-secondary" data-act="rs" data-m="${escapeHtml(u.matricula)}" style="padding:4px 8px;font-size:11px">🔑 Resetar</button>${u.status==="negado"?'<button class="btn-secondary" data-act="ap" data-m="'+escapeHtml(u.matricula)+'" style="padding:4px 8px;font-size:11px">♻ Reativar</button>':(u.role!=="admin"&&!u.owner&&u.matricula!==(CURRENT_USER&&CURRENT_USER.matricula)?'<button class="btn-danger" data-act="bn" data-m="'+escapeHtml(u.matricula)+'" style="padding:4px 8px;font-size:11px">🚫 Banir</button>':'')}</div></div>`).join("");
        p.querySelectorAll("button[data-act]").forEach(b=>{
          b.onclick=async()=>{
